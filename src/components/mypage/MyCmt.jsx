...
import { useComments } from "../../firestore/module/comment";
...
const MyCmt = () => {
  ...
  const { comments, commentsIsLoading } = useComments(localStorage.getItem('uid'));

  const myPosts = posts.filter(post => comments.includes(post.postID));
  const postsLength = myPosts.filter(post => post.contentType === categoryTab).length;
